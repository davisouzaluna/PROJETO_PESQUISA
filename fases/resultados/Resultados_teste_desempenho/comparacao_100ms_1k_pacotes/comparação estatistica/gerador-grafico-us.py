...
def microsec_formatter(x, pos):
    # Convert value from milliseconds to microseconds
    microsec = x * 1000
    return f"{microsec:.2f} µs"
...
def plot_comparison(stats_quic, stats_tcp, stats_tls, stat_name):
    ...
    # For the "mean" graph, remove title and y-label
    if stat_name != 'mean':
    ...
    plt.gca().yaxis.set_major_formatter(FuncFormatter(microsec_formatter))
    plt.gca().yaxis.set_major_locator(MaxNLocator(integer=True, prune='lower'))
    
    for bar in bars:
        yval = bar.get_height()
        plt.text(bar.get_x() + bar.get_width() / 2, yval + 0.00001, 
                 f"{yval * 1000:.2f} µs", ha='center', va='bottom', fontsize=10, color='black')
